...
centroids.to_csv(f"{workdir}/route_9_17_clustered_hdbscan.csv", index=False)
...
clustered_stops = pd.read_csv(f"{workdir}/route_9_17_clustered_hdbscan.csv")
...
# Save final CSV
output_csv = f"{workdir}/cluster_geocoded_route9&17_hdbscan111.csv"
clustered_gdf[['Cluster', 'Lng', 'Lat', 'Address']].to_csv(output_csv, index=False)
print(f"Geocoded data saved to {output_csv}")

# Visualization
fig, ax = plt.subplots(figsize=(10, 10))
stops_gdf.plot(ax=ax, color='blue', marker='o', label="Official Stops", markersize=7)
...
output_img = f"{workdir}/clustering_visualization.png"
